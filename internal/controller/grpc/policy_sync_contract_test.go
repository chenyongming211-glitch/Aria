...
func TestGetQoSRulesIncludesAgentRuntimeContractFields(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)SELECT id, tenant_id, node_id, category.*direction.*rate_bps.*burst_bytes.*priority.*mode.*FROM qos_rules.*WHERE tenant_id = \$1 AND node_id = \$2 AND enabled = true`).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			ruleID, tenantID, nodeID, "service", "10.0.0.0/24", "192.0.2.0/24",
			0, 443, 6, 100, "egress",
