...
func TestPolicyDeliveryErrorResolverUsesPolicyNameForDuplicateRuntimeGroupCIDRConflict(t *testing.T) {
	rawError := "sync failed: sync apply failed: acl_qos: Identity error: Invalid CIDR: duplicate CIDR 100.64.0.2/32 in runtime groups group-a and group-b"
	delivery := &controllerstorage.PolicyDelivery{
		PolicyDomain:  "acl",
		PolicyRef:     "new-acl-rule",
		PolicyName:    "new ACL rule",
		CommandStatus: "failed",
		LastError:     rawError,
	}
	...
	if strings.Contains(lastError, "runtime groups") {
		t.Fatalf("friendly error should not expose runtime group keys, got %q", lastError)
	}
	if rawLastError != rawError {
		t.Fatalf("expected raw_last_error to preserve original error, got %q", rawLastError)
	}
