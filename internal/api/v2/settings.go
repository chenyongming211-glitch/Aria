...
var backupExportTables = []struct {
	...
	{Name: "ip_groups", Query: `SELECT id, tenant_id, name, description, kind, created_by, created_at, updated_at FROM ip_groups ORDER BY created_at ASC`},
	{Name: "ip_group_members", Query: `SELECT id, tenant_id, group_id, cidr, note, created_at FROM ip_group_members ORDER BY created_at ASC`},
	{Name: "acl_rules", Query: `SELECT id, tenant_id, node_id, name, action, src_group_id, dst_group_id, src_cidr, dst_cidr, dst_port, protocol, direction, ports, priority, enabled, description, created_at, updated_at FROM acl_rules ORDER BY created_at ASC`},
	{Name: "qos_rules", Query: `SELECT id, tenant_id, node_id, group_id, src_cidr, dst_cidr, src_port, dst_port, protocol, bandwidth_mbps, direction, rate_bps, burst_bytes, priority, mode, enabled, description, created_at, updated_at FROM qos_rules ORDER BY created_at ASC`},
...
var backupRestoreTables = []backupTableSpec{
	...
	{Name: "ip_groups", Columns: []string{"id", "tenant_id", "name", "description", "kind", "created_by", "created_at", "updated_at"}},
	{Name: "ip_group_members", Columns: []string{"id", "tenant_id", "group_id", "cidr", "note", "created_at"}},
	{Name: "acl_rules", Columns: []string{"id", "tenant_id", "node_id", "name", "action", "src_group_id", "dst_group_id", "src_cidr", "dst_cidr", "dst_port", "protocol", "direction", "ports", "priority", "enabled", "description", "created_at", "updated_at"}},
	{Name: "qos_rules", Columns: []string{"id", "tenant_id", "node_id", "group_id", "src_cidr", "dst_cidr", "src_port", "dst_port", "protocol", "bandwidth_mbps", "direction", "rate_bps", "burst_bytes", "priority", "mode", "enabled", "description", "created_at", "updated_at"}},
...
func validateBackupManifest(manifest *backupManifest, requireRestoreFields bool) error {
	...
	requiredTables := []string{"tenants", "users", "roles", "tokens", "nodes", "ip_groups", "ip_group_members", "acl_rules", "qos_rules", "blacklist_rules"}
