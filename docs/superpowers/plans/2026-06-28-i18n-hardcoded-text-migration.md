...
- `npm run test:run`：28 个测试文件、212 个测试通过。
- `npm run build`：Vite production build 通过，构建产物位于临时 `temp-dist/`，验收后已删除。
- 静态 i18n key 覆盖检查：扫描前端源码中 829 个静态 `t('...')` / `$t('...')` / `titleKey` / `labelKey`，中英文均存在对应翻译，缺失 key 为 0。
- 浏览器抽查：本地预览中文登录页可正常渲染，没有出现 key 泄露。
- 未完成：受保护页面的中英文人工切换抽查需要在线上登录态下完成，本次未伪造登录态。
